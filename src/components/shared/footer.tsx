...
const footerLinks = {
  platform: SITE_CONFIG.tasks.filter((task) => task.enabled && task.key !== 'classified').map((task) => ({
    name: task.label,
    href: task.route,
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr_1fr]">
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-7">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/12 bg-white/8 p-0.5">
                  <img src="/favicon.png?v=20260401" alt={`${SITE_CONFIG.name} logo`} width="48" height="48" className="h-full w-full scale-[1.2] object-contain" />
                </div>
                <div>
                  <p className="text-lg font-semibold">{SITE_CONFIG.name}</p>
  ...
  return (
    <footer className="border-t border-[#73A5CA]/35 bg-[linear-gradient(180deg,#73A5CA_0%,#FFC81E_58%,#E87F24_100%)] text-[#1f2d3a]">
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-10 md:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.8fr]">
          <div>
            <Link href="/" className="flex items-center gap-3">
              <div className="h-11 w-11 overflow-hidden rounded-2xl border border-white/25 bg-white/10 p-0.5 shadow-sm">
                <img src="/favicon.png?v=20260401" alt={`${SITE_CONFIG.name} logo`} width="44" height="44" className="h-full w-full scale-[1.2] object-contain" />
              </div>
              <div>
                <span className="block text-lg font-semibold text-[#1f2d3a]">{SITE_CONFIG.name}</span>
                <span className="text-xs uppercase tracking-[0.22em] text-[#2b3d4f]/80">{siteContent.footer.tagline}</span>
              </div>
            </Link>
            <p className="mt-5 max-w-sm text-sm leading-7 text-[#2b3d4f]/90">{SITE_CONFIG.description}</p>
          </div>
          {(['platform', 'company', 'resources', 'legal'] as const).map((section) => (
            <div key={section}>
              <h3 className="text-sm font-semibold uppercase tracking-[0.22em] text-[#2b3d4f]/85">{section}</h3>
              <ul className="mt-5 space-y-3 text-sm text-[#1f2d3a]/92">
        ...
        <div className="mt-12 border-t border-white/30 pt-6 text-center text-sm text-[#2b3d4f]/85">&copy; {new Date().getFullYear()} {SITE_CONFIG.name}. All rights reserved.</div>
      </div>
    </footer>
  )
}
